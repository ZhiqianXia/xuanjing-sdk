...
class DisabledFrameGenerator : public IFrameGenerator {
 public:
	bool Prepare(std::uint32_t, std::uint32_t, const FrameGenConfig&) override {
		return true;
	}
...
class SimpleBlendFrameGenerator : public IFrameGenerator {
 public:
	bool Prepare(std::uint32_t width, std::uint32_t height,
							 const FrameGenConfig& config) override {
		width_ = width;
		height_ = height;
	...
	bool Generate(const runtime::FrameInput& input,
								runtime::FrameOutput& output) override {
...
IFrameGenerator* CreateDisabledFrameGenerator() {
	return new DisabledFrameGenerator();
}

IFrameGenerator* CreateSimpleBlendFrameGenerator() {
	return new SimpleBlendFrameGenerator();
}
