...
class DisabledTemporalProcessor : public ITemporalProcessor {
 public:
  bool Prepare(std::uint32_t, std::uint32_t, const TemporalConfig&) override { return true; }
...
class SimpleTemporalProcessor : public ITemporalProcessor {
 public:
  bool Prepare(std::uint32_t width, std::uint32_t height, const TemporalConfig& config) override {
    width_ = width;
    height_ = height;
  ...
  bool Process(const runtime::FrameInput& input, TemporalResult& result) override {
    ...
    for (std::uint32_t y = 0; y < height_; ++y) {
      const auto* src = static_cast<const std::uint8_t*>(input.prevHistory.data)
                        + y * input.prevHistory.rowStrideBytes;
      auto* dst = historyBuffer_.data() + static_cast<std::size_t>(y) * width_ * 4U;
      for (std::uint32_t x = 0; x < width_; ++x) {
        const bool reject =
            enableReject_ && input.motionVectors.data != nullptr && input.motionVectors.width > x
            && input.motionVectors.height > y
            && static_cast<const std::uint8_t*>(
                   input.motionVectors.data)[y * input.motionVectors.rowStrideBytes + x * 4]
                   > 220U;
        if (reject) {
          dst[x * 4 + 0] = 0U;
          dst[x * 4 + 1] = 0U;
...
ITemporalProcessor* CreateDisabledTemporalProcessor() { return new DisabledTemporalProcessor(); }

ITemporalProcessor* CreateSimpleTemporalProcessor() { return new SimpleTemporalProcessor(); }
