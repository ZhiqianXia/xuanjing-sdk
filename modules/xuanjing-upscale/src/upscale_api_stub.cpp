...
class PassthroughUpscaler : public IUpscaler {
 public:
  bool Prepare(std::uint32_t, std::uint32_t, std::uint32_t, std::uint32_t, QualityPreset) override {
    return true;
  }
...
class BilinearUpscaler : public IUpscaler {
 public:
  bool Prepare(std::uint32_t inputW, std::uint32_t inputH, std::uint32_t outputW,
               std::uint32_t outputH, QualityPreset) override {
    outputW_ = outputW;
    outputH_ = outputH;
  ...
  bool Upscale(const FrameInput& input, FrameOutput& output) override {
    ...
    for (std::uint32_t y = 0; y < outputH_; ++y) {
      for (std::uint32_t x = 0; x < outputW_; ++x) {
        float u = (x + 0.5f) * static_cast<float>(srcW) / static_cast<float>(outputW_);
        float v = (y + 0.5f) * static_cast<float>(srcH) / static_cast<float>(outputH_);
        ...
        const auto* row0 = static_cast<const std::uint8_t*>(src.data) + y0 * src.rowStrideBytes;
        const auto* row1 = static_cast<const std::uint8_t*>(src.data) + y1 * src.rowStrideBytes;

        std::uint8_t* dst = buffer_.data() + (y * outputW_ + x) * 4;
        for (int c = 0; c < 4; ++c) {
          float p = (1.f - fy) * ((1.f - fx) * row0[x0 * 4 + c] + fx * row0[x1 * 4 + c])
                    + fy * ((1.f - fx) * row1[x0 * 4 + c] + fx * row1[x1 * 4 + c]);
